...
var fight = function(enemy) {
    ...
    while (playerInfo.health>0 && enemy.health>0) { 
        ...
        if (promptFight === "skip" || promptFight === "SKIP") { 
            ...
            if (confirmSkip) {window.alert(playerInfo.name + " has decided to skip this fight. Goodbye!"); 
            //subtract money from playerMoney for skipping
            playerInfo.money = Math.max(0, playerInfo.money - 10) ; console.log("playerInfo.money", playerInfo.money); break; } 
        }
        ...
        console.log (enemy.name + " atacked " + playerInfo.name + ". " + playerInfo.name + " now has " + playerInfo.health + " health remaining.") ;

        // check player's health
        if (playerInfo.health <= 0) {window.alert (playerInfo.name + " has died!"); 
...
var startGame = function() {
    ...
    for (var i=0; i< enemyInfo.length; i=i+1) { 
        ...
        // afther the loop ends, player is either out of health or enemies to fight, so run the endGame function
        endGame();
